...
def export_metrics(history, output_path="metrics.json", extra=None):
    ...
    acc = series("federated_accuracy")
    f1 = series("federated_f1_score")
    precision = series("federated_precision")
    recall = series("federated_recall")
    roc_auc = series("federated_roc_auc")

    payload = {
        "experiment_id": os.environ.get("EXPERIMENT_ID", "federated_run"),
...
def main():
    ...
    export_metrics(history, output_path=os.environ.get("METRICS_PATH", "metrics.json"))
